...
public:
    LatticeFreeMMINode(DEVICEID_TYPE deviceId, const wstring& name)
        : Base(deviceId, name), m_acweight(1.0), m_usePrior(true), m_alignmentWindow(0), m_ceweight(0)
    {
        InitTransitionMatrixes();
    }

    LatticeFreeMMINode(DEVICEID_TYPE deviceId, const wstring& name, const wstring& fstFilePath, const wstring& smapFilePath, const ElemType acweight, bool usePrior, const int alignmentWindow, const ElemType ceweight)
        : Base(deviceId, name), m_acweight(acweight), m_usePrior(usePrior), m_alignmentWindow(alignmentWindow), m_ceweight(ceweight)
    {
        InitTransitionMatrixes();
    ...
    LatticeFreeMMINode(const ScriptableObjects::IConfigRecordPtr configp)
        : LatticeFreeMMINode(configp->Get(L"deviceId"), L"<placeholder>", configp->Get(L"fstFilePath"), configp->Get(L"smapFilePath"), configp->Get(L"acweight"), configp->Get(L"usePrior"), configp->Get(L"alignmentWindow"), configp->Get(L"ceweight"))
    {
    ...
    virtual void BackpropToNonLooping(size_t inputIndex) override
    {
        if (inputIndex == 1)
        {
            ...
            if (m_ceweight == 0)
                Matrix<ElemType>::AddScaledDifference(Gradient() * m_acweight, *m_posteriorsDen, *m_posteriorsNum, gradient);
            else
            {
                // k * (1-alpha) * r_DEN + alpha * P_net - ((1-alpha) * k + alpha) * r_NUM
                m_softmax->InplaceExp();
                Matrix<ElemType>::ScaleAndAdd(m_ceweight, *m_softmax, m_acweight * (1 - m_ceweight), *m_posteriorsDen);
                Matrix<ElemType>::Scale((1 - m_ceweight) * m_acweight + m_ceweight, *m_posteriorsNum);
    ...
    virtual void /*ComputationNodeNonLooping::*/ ForwardPropNonLooping() override
    {
        ...
        double logNumeratorWithCE = CalculateNumeratorsWithCE(labelMatrix, nf);
        ...
        ElemType finalValue = (ElemType)((1 - m_ceweight) * logDenominator - logNumeratorWithCE);
        Value().Resize(1, 1);
    ...
    virtual void DumpNodeInfo(const bool printValues, const bool printMetadata, File& fstream) const override
    {
        if (printMetadata)
        {
            ...
            char str[4096];
            sprintf(str, "acweight=%f usePrior=%d alignmentWindow=%d ceweight=%f", this->m_acweight, this->m_usePrior, this->m_alignmentWindow, this->m_ceweight);
            fstream << string(str);
        }
